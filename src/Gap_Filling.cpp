...
bool Gap_Filling::input_scaffold(const std::string& file) {
...
bool Gap_Filling::input_contigs(const std::string& file) {
    ...
    // cdbg_copy_number.fa
    {
        ...
        while (reader.read(contig)) {
	        Edge edge(curIdx, contig.seq.size(), contig.seq, contig.copy_num);
#ifdef TRAINING
	    ...
	    LOG4CXX_DEBUG(logger, boost::format("all contig number=%d") % all_cdbg_edges.size());
    }
...
bool Gap_Filling::input_debruijn(const std::string& file) {
    ...
    while (reader.read(debruijn)) {
        Edge edge(curIdx, debruijn.seq.size(), debruijn.seq, 0);
        all_initial_edges.push_back(edge);
        ++curIdx;
    }
    LOG4CXX_DEBUG(logger, boost::format("All condensed edges number=%d") % all_initial_edges.size());
}
...
void Gap_Filling::BFS( const  int gap_index, const  int left_index, const  int right_index, int dis)
{
	...
	for(int i = 0 ; i < next.size() ; ++i)
	{
		...
		int len = all_cdbg_edges[index].len - (K-1);
	...
	while(!prev_sets.empty())
	{
		...
		for(int i = 0 ; i < prev_sets.size(); ++i)
		{
			...
			for(int i = 0 ; i < next.size() ; ++i)
			{
				...
				int len2 = len + all_cdbg_edges[node].len-(K-1);
				path.push_back(node);
    ...
    candidates_type::iterator it = tails_initial_hash.find(end);
	...
	for(int i = 0 ; i < it->second.size() ; ++i)
	{
		...
		int len = all_initial_edges[index].len - (K-1);
	...
	while(!prev_sets.empty())
	{
		...
		for(int i = 0 ; i < prev_sets.size(); ++i)
		{
			...
			for(int i = 0 ; i < ALPHABET-1; ++i)
			{
				...
				int len2 = len + all_initial_edges[node].len-(K-1);
				path.push_back(node);
				next_sets.push_back(make_pair(path,len2));
				path.pop_back();
			}
...
void Gap_Filling::get_gap_info() {
    ...
    candidates_type::iterator it;

	//hash initial node
	for (size_t i = 0; i < all_initial_edges.size(); ++i) {
        // prefix
        {
            std::string prefix = all_initial_edges[i].get_prefix();
            char next_base = all_initial_edges[i].get_pre_base();
            int index = Nucleotide::char2code(next_base);
            assert(index != -1);

            candidates_type::iterator it = heads_initial_hash.find(prefix);
            if (it == heads_initial_hash.end()) {
        ...
        // suffix
        {
            std::string suffix = all_initial_edges[i].get_suffix();
            char next_base = all_initial_edges[i].get_suf_base();
            int index = Nucleotide::char2code(next_base);
            assert(index != -1);

            candidates_type::iterator it = tails_initial_hash.find(suffix);
            if (it == tails_initial_hash.end()) {
	...
	LOG4CXX_DEBUG(logger, "Begin build the prev and next node of contig in CDBG...");
	...
	for (size_t i = 0; i < all_initial_edges.size(); ++i) {
        // next
        {
            ...
            std::string suffix = all_initial_edges[i].get_suffix();
            candidates_type::iterator it = heads_initial_hash.find(suffix);
            if (it != heads_initial_hash.end()) {
                assert(it->second.size() == ALPHABET-1);
                for (size_t k = 0 ; k < ALPHABET-1 ; ++k) {
        ...
        // prev
        {
            ...
            std::string prefix = all_initial_edges[i].get_prefix();
            candidates_type::iterator it = tails_initial_hash.find(prefix);
            if (it != tails_initial_hash.end()) {
                assert(it->second.size() == ALPHABET-1);
                for (size_t k = 0 ; k < ALPHABET-1; ++k) {
	...
	LOG4CXX_DEBUG(logger, "Begin build condensed de Buijn graph...");
	...
	for(int i = 0 ; i < all_cdbg_edges.size(); ++i)
	{
		...
		index = Nucleotide::char2code(next_base);
	...
	for (size_t i = 0; i < all_scaffolds.size(); ++i) {
		if (all_scaffolds[i].edge_num() == 1) {
			continue;
		}
        Component::ContigIdList edge_indexs = all_scaffolds[i].contigs;

		for (size_t j = 0; j < edge_indexs.size()-1; ++j) {
			int left_index = edge_indexs.at(j);
			int right_index = edge_indexs.at(j+1);

			gap_indexs.push_back(make_pair(left_index,right_index));
			gap_distances.push_back(all_scaffolds[i].gaps[j]);

            std::string suffix = all_cdbg_edges[left_index].get_suffix();
            std::string prefix = all_cdbg_edges[right_index].get_prefix();
...
void Gap_Filling::uniq_candidate_gap_filling()
{
	...
	for(int i = 0 ;  i < uniq_gap_number ;  ++i )
	{
		...
		if (gap_state.find(cur_gap_index) != gap_state.end())
		{
			...
			{
				assert(all_cdbg_edges[path[j]].len >= K);
		...
		else
		{
			...
			for( int j = 0; j < path.size(); ++j)
			{
				assert(all_initial_edges[path[j]].len >= K);
...
void Gap_Filling::output_initial_scaffolds_seq()
{
	...
	for(int i = 0 ; i < all_scaffolds.size(); ++i)
	{
		...
		vector<size_t> edge_indexs = all_scaffolds[i].contigs;
		...
		for(int j=0 ; j+1 < edge_indexs.size(); ++j)
		{
			...
			int left_len = all_cdbg_edges[left_index].len;
			int right_len = all_cdbg_edges[right_index].len;
