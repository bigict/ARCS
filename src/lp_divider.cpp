...
static log4cxx::LoggerPtr logger(log4cxx::Logger::getLogger("arcs.LPDivider"));
LPDivider LPDivider::_runner;
...
int LPDivider::run(const Properties& options, const Arguments& arguments) {
    ...
    LOG4CXX_DEBUG(logger, "dividerLP begin");
    ...
    LOG4CXX_DEBUG(logger, "divideLP end");
    return r;
}

LPDivider::LPDivider() : Runner("c:s:i:d:h") {
    RUNNER_INSTALL("divideLP", this, "divideLP");
}

int LPDivider::printHelps() const {
    std::cout << "arcs divideLP -i [input] -d [workdir]" << std::endl;
    return 256;
}

int LPDivider::checkOptions(const Properties& options) const {
