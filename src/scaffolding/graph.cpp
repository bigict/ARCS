...
void Graph::addEdge(size_t from, size_t to, long distance, size_t kmer_num, size_t read_num) {
...
void Graph::scoreAndRemoveNoise(const ComponentList& components) {
...
std::ostream& operator<<(std::ostream& os, const Graph& g) {
    size_t index = 0;
    for (Graph::NodeList::const_iterator i = g._graph.begin(); i != g._graph.end(); ++i, ++index) {
        for (Graph::EdgeList::const_iterator j=i->begin(); j != i->end(); ++j) {
            if (j->distance > 0) {
