...
Kmer_Short_Hash_Md5::Kmer_Short_Hash_Md5(unsigned int half_first_alloc):hash_table(NULL), last_alloc_num(half_first_alloc), last_alloc_used(half_first_alloc), prime_pos(0), kmer_count(0),load_factor(0.9), hash_table_used(0){
...
void Kmer_Short_Hash_Md5::push_a_kmer_short(const Kmer &kmer, unsigned int numb){
	...
	Kmer_Short tmp(kmer.first_code, kmer.second_code, kmer.third_code >> 2);
	// kmer.get_pre_node();
	int next = int(kmer.third_code & 0x3);
	...
	for(unsigned long i=0; i<_prime_list[prime_pos]; i++){
		...
		while(tem){
			for(int j = 0; j < 4; j++)
			{
				if(tem->num[j] == 0)
					continue;
				tem->nex[j] = has_kmer(tem->kmer_short.get_next_short(j))->pos;
			}
			tem = tem->next;
		}
	}
