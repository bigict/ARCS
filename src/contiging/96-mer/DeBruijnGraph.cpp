...
void DeBruijnGraph::split_identical_edge()
{
	...
	//cout << "deal identical edge end" << endl;
}

void DeBruijnGraph::divide_edge(int i, int j)
{
	//cout << "divide edge " << i << "\t" << j << endl;
...
void DeBruijnGraph::set_lamada()
{
	...
	// for(unsigned int it = kmer_seq.size() / 2 - 50000; it < kmer_seq.size(); it++ )
	// {
...
void* read_thread(void *arg) {
	...
	unsigned long (*buff)[BUFFER_SIZE][3];
	buff = arg_p->buffer;
	...
	unsigned long first_code, second_code, third_code;
	...
	while(!(*left).eof() && !(*right).eof())
	{
		...
		for(int index = 0; index < int((percent * read1.size() - K + 1)) && index < READ_LENGTH_CUTOFF - K + 1; index ++ )
		{
			...
			for(int i = index+32; i < index+64; i++)
			{
				second_code = (second_code << 2) | ((read1[i] & 0x06) >> 1);
			}
			for(int i = index+64; i < index+K; i ++)
			{
				third_code = (third_code << 2) + ((read1[i] & 0x06) >> 1);
			}
			buff[flag][++count][0] = first_code;
			buff[flag][count][1] = second_code;
			buff[flag][count][2] = third_code;
			first_code = 0;
			second_code = 0;
			third_code = 0;			
			...
			for(int i = lengths - index - K + 32; i < lengths - index - K + 64; i++)
			{
				second_code = (second_code << 2) | ((read1RC[i] & 0x06) >> 1);
			}for(int i = lengths - index - K + 64; i < lengths - index; i++)
			{
				second_code = (third_code << 2) | ((read1RC[i] & 0x06) >> 1);
			}
			buff[flag][++count][0] = first_code;
			buff[flag][count][1] = second_code;
			buff[flag][count][2] = third_code;
		...
		for(int index = 0; index < int((percent * read2.size() - K + 1)) && index < READ_LENGTH_CUTOFF - K + 1; index ++ )
		{
			if(FILTER && (cal_quality_and_min(line2,index,min) < threshold || min < minthres))
				continue;
			first_code = second_code = third_code = 0;
			for(int i = lengths - index - K; i < lengths - index - K + 32; i++)
			{
			...
			for(int i = lengths - index - K + 64; i < lengths - index; i++)
			{
				third_code = (third_code << 2) | ((read2[i] & 0x06) >> 1);
			}
			buff[flag][++count][0] = first_code;
			buff[flag][count][1] = second_code;
			buff[flag][count][2] = third_code;
			...
			for(int i = index+64; i < index+K; i++)
			{
				third_code = (third_code << 2) | ((read2RC[i] & 0x06) >> 1);
			}
			buff[flag][++count][0] = first_code;
			buff[flag][count][1] = second_code;
			buff[flag][count][2] = third_code;
...
void* hash_thread(void *arg) {
	ARG *arg_p = (ARG *) arg;
	unsigned long (*buff)[BUFFER_SIZE][3] = arg_p->buffer;
	pthread_mutex_t *buff1 = arg_p->buffer1;
	pthread_mutex_t *buff2 = arg_p->buffer2;
	pthread_cond_t *cond1 = arg_p->cond1;
	pthread_cond_t *cond2 = arg_p->cond2;
	...
	while(1){
		...
		for(int i=1; i<=count; i++){
			kmer_seq->push_a_kmer(buff[flag][i][0], buff[flag][i][1], buff[flag][i][2]);
		}
		if(flag) {
			buff[1][0][0] = 0;
...
void DeBruijnGraph::initialize_kmer(Lib &lib)
{
	...
	unsigned long (*buffer)[BUFFER_SIZE][3] = new unsigned long[2][BUFFER_SIZE][3];
...
void DeBruijnGraph::initialize_graph()
{
	...
	// for(unsigned int it = 0; it < kmer_short.size(); it++)
	// {
	// 	graph[it].kmer_short = kmer_short[it];
	// }
	...
	// 			graph[it].next[i] = kmer_short.get_kmer_short_pos(next_node);
