...
void Kmer_Hash_Md5::push_a_kmer(unsigned long first, unsigned long second){
	kmer_hashed ++;
	if (kmer_hashed % cou == 0){
		cout << "[INFO] " << kmer_hashed << " kmers have already been hashed..." << endl;
		cou *= 10;
	...
	unsigned long pos = (first  | second)  % _prime_list[prime_pos];
	Kmer_Node_Ptr tem = hash_table[pos];
	...
	if(hash_table[pos] == NULL)
		...
		do{
			if (tem->kmer.first_code == first && tem->kmer.second_code == second){
				tem->num ++;
				return ;
			}
			tem = tem->next;
	...
	Kmer_Node_Ptr tem_node = new( buffer_node[ buffer_node.size()-1 ]+last_alloc_used) Kmer_Node(Kmer(first, second));
...
void Kmer_Hash_Md5::filter2()	// change buffer size
{
	...
	for(int i=0; i<n; i++){
		int m = buffer_size[i];
		int lastm = buffer_size[last_buffer]*1.25;
		for(int j=0; j<m; j++){
			p = buffer_node[i] + j;
			if(p->num > 1){
