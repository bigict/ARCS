...
Kmer_Hash_Md5::Kmer_Hash_Md5(unsigned int half_first_alloc):hash_table(NULL), last_alloc_num(half_first_alloc), 
											...
											load_factor(0.88), hash_table_used(0ul){
...
void Kmer_Hash_Md5::rehash(){
	...
	// fill(tem_table, tem_table+ _prime_list[new_prime_pos], static_cast<Kmer_Node_Ptr>(NULL));
...
//	cout << "[INFO] has_kmer return=NULL" << endl;
	return NULL;
}
Kmer_Node_Ptr* Kmer_Hash_Md5::has_kmer(const Kmer &kmer,bool &flag){
	unsigned long pos = kmer.get_code() % _prime_list[prime_pos];
	Kmer_Node_Ptr tem = hash_table[pos];
	if ( !tem ){
		// flag = false;
...
*/
static int depth = 0;
void Kmer_Hash_Md5::push_a_kmer(unsigned long code){
	kmer_hashed ++;
	if (kmer_hashed % cou == 0){
		cout << "[INFO] " << kmer_hashed << " kmers have already been hashed..." << endl;
		cou *= 10;
	...
	unsigned long pos = code % _prime_list[prime_pos];
	Kmer_Node_Ptr tem = hash_table[pos];
	int count = 0;
	if(tem == NULL)
		hash_table_used ++;
...
//		if(last_alloc_num >= ONEG)
//			last_alloc_num = ONEG;
		Kmer_Node_Ptr buffer = (Kmer_Node_Ptr)malloc( last_alloc_num*sizeof(Kmer_Node));
		cout << "[TEST] void Kmer_Hash_Md5::push_a_kmer(unsigned long) : buffer malloc. (" << last_alloc_num*sizeof(Kmer_Node)/(1024*1024) << "M)" << endl ;
		if (buffer == NULL){
			cout << "bad alloc" << endl;
			exit(1);
		}
	...
	Kmer_Node_Ptr tem_node = new ( buffer_node[ buffer_node.size()-1 ]+last_alloc_used ) Kmer_Node(code);
	buffer_size[ buffer_node.size()-1 ]++;
//	cout << tem_node << endl;
//	cout << "buffer\t" << buffer_node[ buffer_node.size()-1 ]+last_alloc_used << endl;
//	exit(1);
...
void Kmer_Hash_Md5::filter(){
	...
	ofstream out(string("kmers.data").c_str());
	for(unsigned long i=0; i<_prime_list[prime_pos]; i++){
		if (hash_table[i] == NULL){
			continue;
		}
		...
		while(tem){
			if (tem->num < 2){
				...
				out << tem->kmer.first_code << "\t" << tem->num << endl;
				break;
			}
		}
		if (hash_table[i] == NULL){
	...
	cout << "[INFO] Output Kmers.data" << endl;
}
...
void Kmer_Hash_Md5::filter2()	// change buffer size
{
	...
	for(int i=0; i<n; i++){
		int m = buffer_size[i];
		int lastm = buffer_size[last_buffer]*1.5;
		for(int j=0; j<m; j++){
			p = buffer_node[i] + j;
			if(p->num > 1){
				if(index >=lastm){	// 24B -> 16B
