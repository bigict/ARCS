...
for i in range(len(lib_list)):
	...
	ele = lib_list[i]
        ...
        scaffolding_cmd = 'scaffolding/scaffolding -d ' + workspace + ' -K ' + str(kmer_size) + ' -c ' + workspace + '/cdbg_copy_number.fa -e ' + str(edge_length_cutoff) + ' -1 ' + ele[0] + ' -2 ' + ele[1] + ' -L ' + insert_size[i] + ' -P ' + link_quality_percent[i] + ' -p ' + str(cpu_num) + ' -i ' + str(i) + ' -r ' + pair_kmer_cutoff[i] + ' -R ' + pair_reads_cutoff[i] 
	...
	if os.system(remove_repeats_cmd) != 0:
        ...
        #kmer_size -= 2
'''
gap_filling_cmd = 'gap_filling/gap_filling -s scaffold_parameter_0 -K ' + str(kmer_size) + ' -O ' + str(kmer_size - 10) + ' -c cdbg_copy_number.fa -l component_' + str(len(insert_size)) + ' -d ' + workspace + ' -i condensed_de_bruijn_graph_before_trimming.data'
